module.exports = (io, sql, fs, async, app_cfg) => {
  ...
  const waip_speichern = (einsatz_daten) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        sql.db_log(
          "DEBUG",
          "Neuen Einsatz mit der ID " + waip_id + " gespeichert."
        );
        ...
        if (socket_rooms.length > 1) {
          ...
          sql.db_log(
            "Fehler-WAIP",
            "Fehler: Keine Wache für den Einsatz mit der ID " +
              waip_id +
              " vorhanden! Einsatz wird gelöscht!"
          );
        ...
        reject(
          new Error("Fehler beim Speichern der Waip-Einsatzdaten. " + error)
        );
      }
    });
  };
                ...
                if (!result) {
                  ...
                  sql.db_log(
                    ...
                    "Einsatz " +
                      ...
                      " nicht an Socket " +
                      socket.id +
                      " gesendet, da bereits angezeigt (Doppelalarmierung)."
                  );
                }
              }
            );
                    ...
                    sql.db_client_check_waip_id(
                      ...
                      function (same_id) {
                        if (same_id) {
                          ...
                          sql.db_log(
                            "DEBUG",
                            "Rückmeldung JSON: " + JSON.stringify(rmld_obj)
                          );
                        }
                      }
                    );
    ...
    if (typeof socket.id !== "undefined") {
      sql.db_rmld_get_fuer_wache(waip_id, wachen_id, function (rmld_obj) {
        ...
        if (rmld_obj) {
          ...
          sql.db_log(
            "RMLD",
            "Vorhandene Rückmeldungen an Socket " + socket.id + " gesendet."
          );
          sql.db_log("DEBUG", "Rückmeldung JSON: " + JSON.stringify(rmld_obj));
        } else {
          sql.db_log(
            "RMLD",
            "Keine Rückmeldungen für Einsatz-ID" +
              waip_id +
              " und Wachen-ID " +
              wachen_id +
              " vorhanden."
  ...
  function dbrd_verteilen(dbrd_uuid, socket) {
    ...
    sql.db_einsatz_get_by_uuid(dbrd_uuid, function (einsatzdaten) {
      if (einsatzdaten) {
        ...
        sql.db_log(
          "DBRD",
          "Der angefragte Einsatz " +
            dbrd_uuid +
            " ist nicht - oder nicht mehr - vorhanden!, Standby an Socket " +
            socket.id +
            " gesendet."
        );
        sql.db_client_update_status(socket, null);
  ...
  function tts_erstellen(app_cfg, socket_id, einsatzdaten, callback) {
    ...
    if (
      einsatzdaten.einsatzart == "Brandeinsatz" ||
      einsatzdaten.einsatzart == "Hilfeleistungseinsatz"
    ) {
      let mp3_bell = process.cwd() + app_cfg.global.soundpath + "bell_long.mp3";
    } else {
      let mp3_bell =
        process.cwd() + app_cfg.global.soundpath + "bell_short.mp3";
    }
    // Zusammensetzen der Sprachansage
    // TODO db_tts_einsatzmittel -> Resolve bringt nur noch ein Argument!
    async.map(
      JSON.parse(einsatzdaten.em_alarmiert),
      sql.db_tts_einsatzmittel,
      function (err, einsatzmittel) {
        // Grunddaten
        ...
        if (einsatzdaten.objekt) {
          ...
          let tts_text =
            tts_text + ". " + einsatzdaten.ort + ", " + einsatzdaten.ortsteil;
        }
        // Einsatzmittel
        ...
        switch (process.platform) {
          ...
          case "linux":
            ...
            childD.on("exit", function (code, signal) {
              if (code > 0) {
                sql.db_log(
                  "Fehler-TTS",
                  "Exit-Code " + code + "; Fehler beim erstellen der TTS-Datei"
                );
                callback && callback(null);
              } else {
                callback && callback(mp3_url);
          ...
          default:
            sql.db_log(
              "Fehler-TTS",
              "TTS für dieses Server-Betriebssystem nicht verfügbar!"
            );
            callback && callback(null);
        }
      }
  ...
  setInterval(function () {
    ...
    sql.db_einsatz_get_old(app_cfg.global.time_to_delete_waip, function (waip) {
      ...
      if (waip) {
        sql.db_log(
          ...
          "Einsatz mit der ID " +
            waip.id +
            " ist veraltet und kann gelöscht werden."
        );
          ...
          if (socket_ids) {
            socket_ids.forEach(function (row) {
              ...
              if (typeof socket !== "undefined") {
                ...
                sql.db_log(
                  "DBRD",
                  "Dashboard mit dem  Socket " +
                    socket.id +
                    " getrennt, da Einsatz gelöscht."
                );
                sql.db_client_update_status(socket, null);
              }
            });
        ...
        sql.db_einsatz_get_rooms(waip.id, function (data) {
                  ...
                  sql.db_client_check_waip_id(
                    ...
                    function (same_id) {
                      if (same_id) {
                        ...
                        sql.db_log(
                          "WAIP",
                          "Standby an Socket " + socket.id + " gesendet"
                        );
                        sql.db_client_update_status(socket, null);
                      }
                    }
            ...
            function (full_rmld) {
              ...
              sql.db_export_get_recipient(arry_wachen, function (export_data) {
                ...
                if (export_data) {
                  ...
                  let part_rmld = full_rmld.filter((obj) =>
                    String(obj.wache_nr).startsWith(
                      String(export_data.export_filter)
                    )
                  );
                  ...
                  try {
                    ...
                    let csv_filename = export_data.export_name.replace(
                      /[|&;$%@"<>()+,]/g,
                      ""
                    );
                      ...
                      fs.mkdir(
                        ...
                        function (err) {
                          if (err) {
                            sql.db_log(
                              "EXPORT",
                              "Fehler beim Erstellen des Backup-Ordners: " + err
                            );
                          }
                          // CSV speichern
                          fs.writeFile(
                            ...
                            function (err) {
                              if (err) {
                                sql.db_log(
                                  "EXPORT",
                                  "Fehler beim speichern der Export-CSV: " + err
                                );
                              }
                            }
                          );
                    ...
                    if (app_cfg.rmld.backup_to_mail) {
                      ...
                      if (validmail.test(export_data.export_recipient)) {
                        ...
                        let mail_message = {
                          from:
                            "Wachalarm-IP-Web <" + app_cfg.rmld.mail_from + ">",
                          to: export_data.export_recipient,
                          subject:
                            "Automatischer Export Wachalarm-IP-Web - " +
                        ...
                        transport.sendMail(mail_message, function (err, info) {
                          if (err) {
                            sql.db_log(
                              "EXPORT",
                              "Fehler beim senden der Export-Mail an " +
                                export_data.export_recipient +
                                ": " +
                                err
                            );
                          } else {
                        ...
                        sql.db_log(
                          "EXPORT",
                          "Fehler beim versenden der Export-Mail an " +
                            export_data.export_recipient +
                    ...
                    sql.db_log(
                      "EXPORT",
                      "Fehler beim erstellen der Export-CSV: " + err
                    );
                  }
                }
              });
          ...
          sql.db_log(
            "WAIP",
            "Einsatz-Daten zu Einsatz " + waip.id + " gelöscht."
          );
        });
      }
    });
    ...
    fs.readdirSync(process.cwd() + app_cfg.global.soundpath).forEach((file) => {
      ...
      if (
        file.substring(0, 4) != "bell" &&
        file.substring(file.length - 3) == "mp3" &&
        file.substring(file.length - 8) != "_tmp.mp3"
      ) {
        ...
        sql.db_socket_get_by_id(socket_name, function (data) {
          if (!data) {
            fs.unlink(
              process.cwd() + app_cfg.global.soundpath + file,
              function (err) {
                if (err) return sql.db_log("Fehler-WAIP", err);
                sql.db_log(
                  "WAIP",
                  "Veraltete Sound-Datei " + file + " wurde gelöscht."
                );
              }
            );
          }
